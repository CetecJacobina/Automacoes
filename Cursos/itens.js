...
async function obterDadosConteudo(url) {
    try {
        ...
        // Função para verificar se o conteúdo está carregado
        const verificarConteudo = () => doc.querySelector('.note-editable')?.innerHTML.trim() || "Não encontrado";

        // Loop para aguardar o carregamento completo do conteúdo
        let tentativas = 0;

        const conteudo = await new Promise((resolve) => {
            const checkInterval = setInterval(() => {
                tentativas++;
                const conteudoAtual = verificarConteudo();
                if (conteudoAtual !== "Não encontrado") {
                    clearInterval(checkInterval);
                    console.log(`📜 Conteúdo encontrado:\n${conteudoAtual.substring(0, 50)}...`);
                    resolve(conteudoAtual);
                } else if (tentativas >= 15) {  // Tentar por 15 segundos
                    clearInterval(checkInterval);
                    console.log('⚠️ Conteúdo não encontrado após múltiplas tentativas.');
